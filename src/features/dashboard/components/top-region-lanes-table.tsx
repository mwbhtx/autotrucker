...
const LANE_FIELD: Record<AnalyticsTopLanesSort, keyof AnalyticsTopRegionLaneEntry> = {
...
export function TopRegionLanesTable({
  companyId,
  from,
  to,
}: {
  ...
  const { data, isLoading, isError } = useAnalyticsTopRegionLanes(companyId, from, to);
          ...
          <Table>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className={sort === "loads_per_day" ? "opacity-100" : "opacity-0"}>▼</span>
                      </button>
                    </TooltipTrigger>
                    <TooltipContent className="max-w-xs">{OPENED_PER_DAY_TOOLTIP}</TooltipContent>
                  </Tooltip>
                </TableHead>
                <TableHead className="text-right">
                  <button
                  ...
                  >
                    <span>$/mi</span>
                    <span className={sort === "rate_per_mile" ? "opacity-100" : "opacity-0"}>▼</span>
                  </button>
                </TableHead>
                <TableHead className="text-right">
                  <button
                  ...
                  >
                    ...
                    <span className={sort === "median_pay" ? "opacity-100" : "opacity-0"}>▼</span>
                  </button>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                rows.map((r, i) => (
                  <TableRow key={`${r.origin_cell_lat},${r.origin_cell_lng}->${r.destination_cell_lat},${r.destination_cell_lng}`}>
